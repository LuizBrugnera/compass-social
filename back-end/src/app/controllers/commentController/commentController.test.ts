...
describe('Comment Controller', () => {
  ...
  test('should get all comments of a post', async () => {
    const response = await request(app).get(`/api/test/posts/${postId}/comments`);
  ...
  test('should get a comment of a post by id', async () => {
    const response = await request(app).get(`/api/test/posts/${postId}/comments/${commentId}`);
  ...
  test('should delete a comment', async () => {
    const response = await request(app).delete(`/api/test/posts/${postId}/comments/${commentId}`);

    expect(response.statusCode).toBe(200);
    expect(response.body.msg).toBe('Comment deleted successfully');
    expect(response.body.response._id).toBe(commentId);
    console.log(response.body)
  });
});